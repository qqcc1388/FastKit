...
- (NSString *)descriptionTimeBetweenNow
{
    ...
    if (days > 365) {
        des = [NSString stringWithFormat:LGGetStringWithKeyAndDefult(@"%@年前",@"%@年前"), @(days/365)];
    }
    else if (days > 30) {
        des = [NSString stringWithFormat:LGGetStringWithKeyAndDefult(@"%@个月前",@"%@个月前"), @(days/30)];
    }
    else if (days > 7) {
        des = LGGetStringWithKeyAndDefult(@"一周前",@"一周前");
    }
    else if (days > 2) {
        des = [NSString stringWithFormat:LGGetStringWithKeyAndDefult(@"%@天前",@"%@天前"), @(days)];
    }
    else if (days == 2){
        des = LGGetStringWithKeyAndDefult(@"前天",@"前天");
    }
    else if (days == 1){
        des = LGGetStringWithKeyAndDefult(@"昨天",@"昨天");
    }
    else {
        ...
        if (duration > secPerHour) {
            des = [NSString stringWithFormat:LGGetStringWithKeyAndDefult(@"%@小时前",@"%@小时前"), @(duration/secPerHour)];
        }
        else if (duration > 120){
            des = [NSString stringWithFormat:LGGetStringWithKeyAndDefult(@"%@分钟前",@"%@分钟前"), @(duration/60)];
        }
        else {
            des = LGGetStringWithKeyAndDefult(@"刚刚",@"刚刚");
        }
    }
