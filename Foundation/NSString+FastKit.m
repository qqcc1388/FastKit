...
//    @"^((1[3,4,5,7,8][0-9])|(15[^4,\\D])|(18[0,5-9]))\\d{8}$"
    NSString *regex = @"^((13[0-9])|(14[5,7,9])|(15[0-3,5-9])|(17[0,1,3,5-8])|(18[0-9]))\\d{8}$";
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", regex];
    return [predicate evaluateWithObject:self];
}

+ (BOOL)isMobilePhone:(NSString*)phone {
    return [phone isMobilePhone];
}
...
- (BOOL)isBankCardNo {
    ...
    for (int i=0; i< forwardDescArr.count; i++) {
        ...
        if (i%2) {//偶数位
            [arrEvenNum addObject:@(num)];
        }
        else{//奇数位
            if (num * 2 < 9) {
                [arrOddNum addObject:@(num * 2)];
            }
            else{
                NSInteger decadeNum = (num * 2) / 10;
                NSInteger unitNum = (num * 2) % 10;
                [arrOddNum2 addObject:@(unitNum)];
                [arrOddNum2 addObject:@(decadeNum)];
            }
        }
    }
...
+ (BOOL)isPersonIDCardNo:(NSString *)idCard {
    return [idCard isPersonIDCardNo];
}
...
@implementation NSMutableString (FastKit)
